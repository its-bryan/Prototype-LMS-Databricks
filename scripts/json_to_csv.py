...
- Nested keys are flattened with dot notation (e.g., A_basic_context.insurer_name)
...
Usage:
    python scripts/json_to_csv.py                          # Process default directory
    python scripts/json_to_csv.py --input /path/to/json    # Custom input directory
    python scripts/json_to_csv.py --output results.csv     # Custom output filename
    python scripts/json_to_csv.py --dry-run                # Show what would be processed

Input:  data/audio 2/JSON/*.json (default)
Output: data/audio 2/JSON/combined_extractions.csv (default)
"""
...
DEFAULT_INPUT_DIR = PROJECT_ROOT / "data" / "audio 2" / "JSON"
DEFAULT_OUTPUT_FILE = "combined_extractions.csv"

# Expected top-level sections in transcript extraction JSON
EXPECTED_SECTIONS = [
