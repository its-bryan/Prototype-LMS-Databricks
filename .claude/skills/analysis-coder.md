...
```
/analysis-coder <hypothesis_id> <test_type> <columns>
```

Or provide context naturally:
```
/analysis-coder H1: test if contact_range affects conversion using chi-square on columns [contact_range, rental] from df_conversion
```

## Input Parameters

- **hypothesis_id**: Reference to hypothesis (e.g., H1, H2)
- **test_type**: Statistical test to run (from stat-test-selector)
- **columns**: List of column names [predictor, outcome]
- **dataframe**: Name of the dataframe (default: `df`)
- **filters**: Optional data filters to apply

## Code Templates

### Chi-Square Test (Categorical × Binary)
...
### Logistic Regression (Continuous/Categorical → Binary)
...
### Conversion Rate Comparison (Simple)
...
### Correlation Analysis (Continuous × Continuous/Binary)
...
The skill generates:
1. Complete, runnable Python function
2. Docstring explaining the hypothesis being tested
3. Results dictionary with all key statistics
4. Print statements for quick interpretation
5. Effect size measures appropriate to the test

## Notes

- All functions handle missing data by dropping NaN values
- Functions return dictionaries for easy integration into reports
- Significance threshold is α = 0.05 by default
- Effect size interpretations follow Cohen's conventions where applicable
