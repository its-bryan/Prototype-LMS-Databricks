...
export default function InteractiveInbox() {
  ...
  if (loading) {
    return (
      <div>
        <h2 className="text-xl font-semibold text-[var(--hertz-black)] mb-4">Inbox</h2>
  ...
  return (
    <div>
      <div className="flex items-center gap-4 mb-1">
        <h2 className="text-xl font-semibold text-[var(--hertz-black)]">Inbox</h2>
        {total > 0 && (
        ...
        <div className="space-y-3 max-w-3xl">
          {directiveLeads.map((lead, i) => (
            <Motion.div
              ...
              className="border border-[var(--neutral-200)] rounded-lg p-4 cursor-pointer hover:border-[var(--hertz-primary)] hover:shadow-sm transition-all group"
            >
              <div className="flex items-center justify-between mb-2">
                <div className="flex items-center gap-3">
                  <span className="text-sm font-medium text-[var(--hertz-black)]">{lead.customer}</span>
                  <span className="text-xs font-mono text-[var(--neutral-600)]">{lead.reservationId}</span>
                </div>
                <div className="flex items-center gap-2">
                  <StatusBadge status={lead.status} />
                  {!lead.enrichmentComplete && (
                    <span className="text-xs text-[var(--color-error)] font-medium">Needs comments</span>
                  )}
                </div>
              </div>
              <div className="flex items-start gap-2">
                ...
                <p className="text-sm text-[var(--hertz-black)] leading-relaxed">{lead.gmDirective}</p>
              </div>
              <div className="flex items-center gap-3 mt-2 text-xs text-[var(--neutral-600)]">
                {lead.initDtFinal && (
