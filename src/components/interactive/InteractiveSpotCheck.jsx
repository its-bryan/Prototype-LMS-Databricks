...
export default function InteractiveSpotCheck() {
  ...
  const untouchedLeads = getUntouchedLeadsForBranch(selectedBranch);
    ...
    <div>
      ...
      <div className="mb-4">
        ...
        <select
          ...
          className="border border-[#E6E6E6] rounded px-3 py-2 text-sm bg-white focus:border-[#F5C400] focus:outline-none"
        >
          {branches.map((b) => (
      ...
      >
        ...
        <textarea
          ...
          className="w-full border border-[#F5C400] bg-yellow-50 rounded px-3 py-2 text-sm focus:outline-none resize-none"
        />
        <div className="flex items-center gap-3 mt-2">
          <button
            onClick={handleSend}
            disabled={!directive.trim()}
            className="px-4 py-1.5 bg-[#F5C400] text-[#1A1A1A] rounded text-sm font-medium hover:bg-[#e0b200] transition-colors cursor-pointer disabled:opacity-40 disabled:cursor-not-allowed"
          >
            Send Directive
          </button>
          {isSent && (
